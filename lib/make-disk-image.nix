/*
  ...
  This version adds support for cross-compiling disk images (e.g., building aarch64-linux
  images on x86_64-linux) by registering binfmt handlers inside the build VM.

  Set `targetSystem` to the target architecture (e.g., "aarch64-linux") and provide
  `binfmtInterpreter` pointing to a static QEMU user-mode binary (e.g., pkgsStatic.qemu-user).
*/
{
  pkgs,
  lib,
  ...
  # --- Cross-compilation support ---
  # Target system for the image (e.g., "aarch64-linux").
  # If different from the build system, binfmt will be used.
  targetSystem ? null,

  # Static QEMU user-mode emulator package for cross-compilation.
  # Should be a static build like pkgsStatic.qemu-user.
  # Required when targetSystem differs from the build system.
  binfmtInterpreter ? null,
}:
...
# Cross-compilation requires binfmt interpreter
assert (
  lib.assertMsg (
    targetSystem != null -> binfmtInterpreter != null
  ) "Cross-compilation (targetSystem != null) requires binfmtInterpreter to be set."
);

let
  format' = format;

  # Determine QEMU architecture name from NixOS system name
  qemuArchMap = {
  ...
  qemuArch = if targetSystem != null then qemuArchMap.${targetSystem} or null else null;

  # binfmt magic and mask for the target system
  # These are the canonical values from nixpkgs/nixos/lib/binfmt-magics.nix
  binfmtMagics = {
    "aarch64-linux" = {
  ...
  binfmtMagic = if targetSystem != null then binfmtMagics.${targetSystem} or null else null;

  # Script to register binfmt handler for cross-compilation
  binfmtSetup = lib.optionalString (targetSystem != null && binfmtMagic != null) ''
    echo "Setting up binfmt for cross-compilation (target: ${targetSystem})..."
    # Mount binfmt_misc filesystem
    mount -t binfmt_misc binfmt_misc /proc/sys/fs/binfmt_misc 2>/dev/null || true
    # Register the handler with F flag (fix binary) so it works in chroots
    echo ':${targetSystem}:M::${binfmtMagic.magic}:${binfmtMagic.mask}:${binfmtInterpreter}/bin/qemu-${qemuArch}:PF' > /proc/sys/fs/binfmt_misc/register || {
      echo "Warning: Failed to register binfmt handler"
      cat /proc/sys/fs/binfmt_misc/status || true
    }
    echo "binfmt registration complete"
  '';
    ...
    pkgs.runCommand name
      {
        ...
        buildInputs =
          ...
          ++ lib.optional (binfmtInterpreter != null) binfmtInterpreter;
      ...
      ''
        ...
        ${lib.optionalString installBootLoader ''
          ...
          # Set up binfmt for cross-compilation if needed
          ${binfmtSetup}
